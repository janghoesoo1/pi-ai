...
> 출처: harness_engineering_qa (황민호 수석 Q&A) "비용 vs 품질 트레이드오프"

에이전트/작업별로 모델과 추론 레벨을 지정할 수 있다(Anthropic도 권장). 단 검증 체계 없이 개인이 모델 티어를 판단하기는 어렵다.

- **검증 체계가 없거나 잘 모르면 → 가장 좋은 모델을 쓴다.** (섣부른 다운그레이드가 품질 비용을 키운다)
- **추론 레벨은 high 또는 xhigh 권장.** `xhigh`는 `high` 대비 약 4배 비용·시간. `max`는 너무 오래 걸려 실무 비권장.
- **티어를 낮추려면 먼저 검증 체계(eval, 교차검증)부터 세운다.** 교차검증은 `prompts/19-cross-verify.md` 참조.
- L4(린트·포맷·빌드검증) 같은 결정론적 작업은 낮은 티어로 내려도 안전. L1/L2 판단 작업은 best 모델 유지.
